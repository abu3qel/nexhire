...
export default function RecruiterLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!loading && (!user || user.role !== "recruiter")) router.push("/login");
  }, [user, loading, router]);
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      ...
      <div className="px-5 pt-4 pb-2">
        <span className="text-[10px] font-semibold uppercase tracking-widest text-white/25">Recruiter</span>
      </div>
      ...
      <div className="px-3 pb-4 mt-4 border-t border-white/[0.06] pt-4">
        <div className="flex items-center gap-3 px-3 py-2 rounded-lg">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-xs text-white/35 truncate leading-tight">{user.company_name || user.email}</div>
          </div>
        </div>
        <button
          onClick={logout}
  ...
  return (
    <div className="min-h-screen bg-[#F5F6FA] lg:flex">
      ...
      <aside className={`
        fixed inset-y-0 left-0 z-50 w-56 bg-[#0D1117] flex flex-col
