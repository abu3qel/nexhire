...
export default function JobDetailPage() {
  ...
  return (
    <div>
      ...
      <div className="flex items-start gap-4 mb-6">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-3 flex-wrap">
            <h1 className="font-sora text-xl font-bold text-white">{job.title}</h1>
      ...
      {/* Tabs */}
      <div className="flex gap-1 mb-4 bg-[#111827] rounded-xl p-1 w-fit">
        {TABS.map((tab, i) => (
          <button
            key={tab}
            onClick={() => setActiveTab(i)}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
              activeTab === i ? "bg-[#1f2937] text-white" : "text-gray-500 hover:text-gray-300"
            }`}
          >
            {tab}
          </button>
      ...
      <Card>
        {rankLoading ? (
          ...
          <>
            ...
            {activeTab === 2 && <BaselineComparison candidates={ranked} />}
          </>
        )}
      </Card>
    </div>
